...
template <size_t Dim>
SoStandingWave<Dim>::SoStandingWave(CkMigrateMessage* msg)
    : InitialData(msg) {}
...
SoStandingWave<Dim>::variables(
  ...
  auto phi = make_with_value<tnsr::i<DataVector, Dim, Frame::Inertial>>(x,
                                                                        0.0);
  for (size_t d = 0; d < Dim; ++d) {
...
SoStandingWave<Dim>::variables(
  ...
  return {std::move(get<Tags::Psi>(base_vars)),
          ...
          std::move(get<Tags::Phi<Dim>>(base_vars)),
          std::move(boundary_psi)};
}
...
bool operator==(const SoStandingWave<Dim>& lhs,
  ...
  return (lhs.wave_vector_ == rhs.wave_vector_) and
         ...
         (lhs.amplitude_ == rhs.amplitude_) and (lhs.omega_ == rhs.omega_);
}
