...
HighSpinKerrPuncture::IntermediateVars<DataType>::IntermediateVars(
    ...
    : mass(mass_in),
      ...
      r_plus(mass_in + sqrt(square(mass_in) - square(spin_a))),
      r_minus(mass_in - sqrt(square(mass_in) - square(spin_a))) {
  ...
  // Boyer-Lindquist radius and its r-derivative, LES Eq. (11):
  //   r_BL = r (1 + r_+/4r)^2 = r + r_+/2 + r_+^2/(16 r),
  //   r_BL' = 1 - r_+^2/(16 r^2).
  r_bl = r + 0.5 * r_plus + square(r_plus) / 16. * one_over_r;
  d_r_bl_d_r = 1. - square(r_plus) / 16. * square(one_over_r);

  // r_BL - r_-, kept as a named intermediate; it is > 0 for all r > 0.
  r_bl_minus_r_minus = r_bl - r_minus;
  ...
  //   Delta = (r_BL - r_+)(r_BL - r_-), via the factored identity
  //           r_BL - r_+ = (r - r_+/4)^2 / r to preserve exact nonnegativity.
  ...
  //                       Sigma^{-3/2} r^{-13/2}.
  // The two vanishing factors (r - r_+/4) and z are kept explicit so the
  // coefficient and its derivatives avoid 0/0 at the throat and equator.
  ...
  // Lapse factor g = sqrt((r_BL - r_-) Sigma / (r A)); the LES lapse is
  // alpha = |r - r_+/4| g.
