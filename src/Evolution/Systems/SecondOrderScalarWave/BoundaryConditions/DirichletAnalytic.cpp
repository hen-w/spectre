  ...
  auto boundary_values = call_with_dynamic_type<
      ...
      [&coords, &time](const auto* const analytic_solution_or_data) {
        if constexpr (is_analytic_solution_v<
          ...
          (void)time;
          return analytic_solution_or_data->variables(
              coords, tmpl::list<SecondOrderScalarWave::Tags::Psi,
                                 SecondOrderScalarWave::Tags::Pi,
                                 SecondOrderScalarWave::Tags::Phi<Dim>>{});
        }
      });
  *psi = get<SecondOrderScalarWave::Tags::Psi>(boundary_values);
  *pi = get<SecondOrderScalarWave::Tags::Pi>(boundary_values);
  *phi = get<SecondOrderScalarWave::Tags::Phi<Dim>>(boundary_values);
