...
template <typename Frame>
db::prefix_variables<::Tags::dt, typename Tags::CharacteristicFields<
                                     DataVector, Dim, Frame>::type>
dt_characteristic_fields(
    const tnsr::i<DataVector, Dim, Frame>& unit_normal_one_form,
    const tnsr::ii<DataVector, Dim, Frame>& conformal_spatial_metric,
    const Scalar<DataVector>& conformal_factor, const Scalar<DataVector>& lapse,
    const tnsr::I<DataVector, Dim, Frame>& shift,
    const Scalar<DataVector>& dt_trace_extrinsic_curvature,
    const tnsr::ii<DataVector, Dim, Frame>& dt_a_tilde,
    const Scalar<DataVector>& dt_theta,
    const tnsr::I<DataVector, Dim, Frame>& dt_gamma_hat,
    const tnsr::I<DataVector, Dim, Frame>& dt_auxiliary_field_b,
    const tnsr::ijj<DataVector, Dim, Frame>& dt_d_conformal_spatial_metric,
    const tnsr::i<DataVector, Dim, Frame>& dt_d_conformal_factor,
    const tnsr::i<DataVector, Dim, Frame>& dt_d_lapse,
    const tnsr::iJ<DataVector, Dim, Frame>& dt_d_shift, const double f) {
  auto dt_char_fields = make_with_value<db::prefix_variables<
      ::Tags::dt,
      typename Tags::CharacteristicFields<DataVector, Dim, Frame>::type>>(
      get(lapse), 0.);
  const auto temp_dt_char_fields = characteristic_fields(
  ...
  tmpl::for_each<typename Tags::CharacteristicFields<DataVector, Dim,
                                                     Frame>::type::tags_list>(
      [&]<typename Tag>(tmpl::type_<Tag> /*meta*/) {
        get<::Tags::dt<Tag>>(dt_char_fields) = get<Tag>(temp_dt_char_fields);
      });
  return dt_char_fields;
}
...
template <typename Frame>
db::prefix_variables<::Tags::dt,
                     typename Tags::EvolvedSpaceFromCharacteristicFields<
                         DataVector, Dim, Frame>::type>
dt_evolved_space_from_dt_characteristic_fields(
    const tnsr::ii<DataVector, Dim, Frame>& dt_u_tnsr_plus,
    const tnsr::ii<DataVector, Dim, Frame>& dt_u_tnsr_minus,
    const tnsr::i<DataVector, Dim, Frame>& dt_u_vector1_zero,
    const tnsr::i<DataVector, Dim, Frame>& dt_u_vector2_plus,
    const tnsr::i<DataVector, Dim, Frame>& dt_u_vector2_minus,
    const tnsr::i<DataVector, Dim, Frame>& dt_u_vector3_plus,
    const tnsr::i<DataVector, Dim, Frame>& dt_u_vector3_minus,
    const Scalar<DataVector>& dt_u_scalar1_zero,
    const Scalar<DataVector>& dt_u_scalar2_plus,
    const Scalar<DataVector>& dt_u_scalar2_minus,
    const Scalar<DataVector>& dt_u_scalar3_plus,
    const Scalar<DataVector>& dt_u_scalar3_minus,
    const Scalar<DataVector>& dt_u_scalar4_plus,
    const Scalar<DataVector>& dt_u_scalar4_minus,
    const Scalar<DataVector>& dt_u_scalar5_plus,
    const Scalar<DataVector>& dt_u_scalar5_minus,
    const tnsr::i<DataVector, Dim, Frame>& unit_normal_one_form,
    const tnsr::ii<DataVector, Dim, Frame>& conformal_spatial_metric,
    const Scalar<DataVector>& conformal_factor, const Scalar<DataVector>& lapse,
    const tnsr::I<DataVector, Dim, Frame>& shift, double f) {
  auto dt_evolved_space = make_with_value<db::prefix_variables<
      ::Tags::dt, typename Tags::EvolvedSpaceFromCharacteristicFields<
                      DataVector, Dim, Frame>::type>>(get(lapse), 0.);
  const auto temp_dt_evolved_space = evolved_space_from_characteristic_fields(
      dt_u_tnsr_plus, dt_u_tnsr_minus, dt_u_vector1_zero, dt_u_vector2_plus,
      dt_u_vector2_minus, dt_u_vector3_plus, dt_u_vector3_minus,
      dt_u_scalar1_zero, dt_u_scalar2_plus, dt_u_scalar2_minus,
      dt_u_scalar3_plus, dt_u_scalar3_minus, dt_u_scalar4_plus,
      dt_u_scalar4_minus, dt_u_scalar5_plus, dt_u_scalar5_minus,
      unit_normal_one_form, conformal_spatial_metric, conformal_factor, lapse,
      shift, f);
  tmpl::for_each<typename Tags::EvolvedSpaceFromCharacteristicFields<
      DataVector, Dim, Frame>::type::tags_list>(
      [&]<typename Tag>(tmpl::type_<Tag> /*meta*/) {
        get<::Tags::dt<Tag>>(dt_evolved_space) =
            get<Tag>(temp_dt_evolved_space);
      });
  return dt_evolved_space;
}
...
#define INSTANTIATION(_, data)                                                 \
  ...
  template db::prefix_variables<                                               \
      ::Tags::dt, typename Ccz4::fd::Tags::CharacteristicFields<               \
                      DataVector, Ccz4::fd::Dim, FRAME(data)>::type>           \
  Ccz4::fd::dt_characteristic_fields<FRAME(data)>(                             \
      const tnsr::i<DataVector, Ccz4::fd::Dim, FRAME(data)>&                   \
          unit_normal_one_form,                                                \
      const tnsr::ii<DataVector, Ccz4::fd::Dim, FRAME(data)>&                  \
          conformal_spatial_metric,                                            \
      ...
      const tnsr::I<DataVector, Ccz4::fd::Dim, FRAME(data)>& shift,            \
      const Scalar<DataVector>& dt_trace_extrinsic_curvature,                  \
      const tnsr::ii<DataVector, Ccz4::fd::Dim, FRAME(data)>& dt_a_tilde,      \
      const Scalar<DataVector>& dt_theta,                                      \
      const tnsr::I<DataVector, Ccz4::fd::Dim, FRAME(data)>& dt_gamma_hat,     \
      const tnsr::I<DataVector, Ccz4::fd::Dim, FRAME(data)>&                   \
          dt_auxiliary_field_b,                                                \
      const tnsr::ijj<DataVector, Ccz4::fd::Dim, FRAME(data)>&                 \
          dt_d_conformal_spatial_metric,                                       \
      const tnsr::i<DataVector, Ccz4::fd::Dim, FRAME(data)>&                   \
          dt_d_conformal_factor,                                               \
      const tnsr::i<DataVector, Ccz4::fd::Dim, FRAME(data)>& dt_d_lapse,       \
      const tnsr::iJ<DataVector, Ccz4::fd::Dim, FRAME(data)>& dt_d_shift,      \
      const double f);                                                         \
  ...
  template db::prefix_variables<                                               \
      ...
      typename Ccz4::fd::Tags::EvolvedSpaceFromCharacteristicFields<           \
          DataVector, Ccz4::fd::Dim, FRAME(data)>::type>                       \
  Ccz4::fd::dt_evolved_space_from_dt_characteristic_fields<FRAME(data)>(       \
      const tnsr::ii<DataVector, Ccz4::fd::Dim, FRAME(data)>& dt_u_tnsr_plus,  \
      const tnsr::ii<DataVector, Ccz4::fd::Dim, FRAME(data)>& dt_u_tnsr_minus, \
      const tnsr::i<DataVector, Ccz4::fd::Dim, FRAME(data)>&                   \
          dt_u_vector1_zero,                                                   \
      const tnsr::i<DataVector, Ccz4::fd::Dim, FRAME(data)>&                   \
          dt_u_vector2_plus,                                                   \
      ...
      const Scalar<DataVector>& conformal_factor,                              \
      const Scalar<DataVector>& lapse,                                         \
      const tnsr::I<DataVector, Ccz4::fd::Dim, FRAME(data)>& shift, double f);
