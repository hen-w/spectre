...
static void apply(
  ...
  *lapse_times_slicing_condition =
  ...
  // eq 13
  ::tenex::evaluate(trace_a_tilde,
                    0.0 * (*inv_conformal_spatial_metric)(ti::I, ti::J) *
                        a_tilde(ti::i, ti::j));
  ...
  static void apply(const gsl::not_null<db::DataBox<DbTagsList>*> box) {
    ...
    for (const auto& direction : element.external_boundaries()) {
      ...
      if (direction != Direction<Dim>::upper_zeta()) {
        ERROR(
            "Sommerfeld and ConstraintRadiationPreserving BCs are only "
            "implemented assuming"
            " the outer boundary is in the upper zeta direction, but the "
            "current "
            "external boundary is in direction "
            << direction);
      }
    }
    ...
    if (dynamic_cast<const Ccz4::BoundaryConditions::Sommerfeld*>(
      ...
      db::mutate<dt_variables_tag>(
          [&](const auto dt_vars_ptr, const auto& inertial_coords,
            ...
            tmpl::for_each<Ccz4::fd::System::gradients_tags>(
                [&]<typename Tag>(tmpl::type_<Tag> /*meta*/) {
                  const auto& var = get<Tag>(evolved_vars);
                  const auto& d_var = get<
                      ::Tags::deriv<Tag, tmpl::size_t<Dim>, Frame::Inertial>>(
                      cell_centered_Ccz4_derivs);
                  auto& dt_var = get<::Tags::dt<Tag>>(*dt_vars_ptr);

                  for (size_t tensor_index = 0;
                       tensor_index < Tag::type::size(); ++tensor_index) {
    ...
    // Handling ConstraintRadiationPreserving BCs
    else if (const auto* crpbc_ptr =
                 dynamic_cast<const Ccz4::BoundaryConditions::
      ...
      db::mutate<dt_variables_tag>(
          [&](const auto dt_vars_ptr, const auto& inertial_coords,
            ...
            // Slice into the outermost layer of the evolved variables and their
            // derivatives
            Variables<Ccz4::fd::System::variables_tag_list>
                outermost_evolved_vars{num_face_pts};
            Variables<deriv_var_tag> outermost_Ccz4_derivs{num_face_pts};
            Variables<typename dt_variables_tag::tags_list> outermost_dt_vars{
                num_face_pts};

            tmpl::for_each<Ccz4::fd::System::
                               gradients_tags>([&]<typename Tag>(
                                                   tmpl::type_<Tag> /*meta*/) {
              const auto& var = get<Tag>(evolved_vars);
              ...
              for (size_t tensor_index = 0; tensor_index < Tag::type::size();
                   ++tensor_index) {
                make_const_view<DataVector>(
                    make_not_null(&outermost_var[tensor_index]),
                    var[tensor_index],
                    (subcell_mesh.extents(2) - 1) * num_face_pts, num_face_pts);
            ...
            // Compute unit_normal_vector and unit_normal_one_form at the
            // outermost layer
            ...
            const auto& conformal_factor_squared =
                get<::Ccz4::Tags::ConformalFactorSquared<DataVector>>(
                    temp_vars);
            ...
            for (size_t i = 0; i < Dim; ++i) {
              unit_normal_vector.get(i) /= get(magnitude);
            }
            const tnsr::i<DataVector, Dim> unit_normal_one_form =
                raise_or_lower_index(unit_normal_vector,
                                     outermost_spatial_metric);

            // Compute the unmodified evolution of characteristic fields
            tnsr::ijj<DataVector, Dim> outermost_dt_d_conformal_spatial_metric;
            tnsr::i<DataVector, Dim> outermost_dt_d_conformal_factor;
            tnsr::i<DataVector, Dim> outermost_dt_d_lapse;
            tnsr::iJ<DataVector, Dim> outermost_dt_d_shift;

            tnsr::iijj<DataVector, Dim> outermost_d_d_conformal_metric;
            for (size_t tensor_index = 0;
                 tensor_index < outermost_d_d_conformal_metric.size();
                 ++tensor_index) {
              make_const_view<DataVector>(
                  make_not_null(&outermost_d_d_conformal_metric[tensor_index]),
                  d_d_conformal_metric[tensor_index],
                  (subcell_mesh.extents(2) - 1) * num_face_pts, num_face_pts);
            }
            tnsr::ii<DataVector, Dim> outermost_d_d_conformal_factor;
            for (size_t tensor_index = 0;
                 tensor_index < outermost_d_d_conformal_factor.size();
                 ++tensor_index) {
              make_const_view<DataVector>(
                  make_not_null(&outermost_d_d_conformal_factor[tensor_index]),
                  d_d_conformal_factor[tensor_index],
                  (subcell_mesh.extents(2) - 1) * num_face_pts, num_face_pts);
            }
            tnsr::ii<DataVector, Dim> outermost_d_d_lapse;
            for (size_t tensor_index = 0;
                 tensor_index < outermost_d_d_lapse.size(); ++tensor_index) {
              make_const_view<DataVector>(
                  make_not_null(&outermost_d_d_lapse[tensor_index]),
                  d_d_lapse[tensor_index],
                  (subcell_mesh.extents(2) - 1) * num_face_pts, num_face_pts);
            }
            tnsr::iiJ<DataVector, Dim> outermost_d_d_shift;
            for (size_t tensor_index = 0;
                 tensor_index < outermost_d_d_shift.size(); ++tensor_index) {
              make_const_view<DataVector>(
                  make_not_null(&outermost_d_d_shift[tensor_index]),
                  d_d_shift[tensor_index],
                  (subcell_mesh.extents(2) - 1) * num_face_pts, num_face_pts);
            }
            ...
            const auto& outermost_a_tilde =
                get<::Ccz4::Tags::ATilde<DataVector, Dim>>(
                    outermost_evolved_vars);
            const auto& outermost_theta =
                get<::Ccz4::Tags::Theta<DataVector>>(outermost_evolved_vars);
            const auto& outermost_lapse =
                get<gr::Tags::Lapse<DataVector>>(outermost_evolved_vars);
            const auto& outermost_shift =
                get<gr::Tags::Shift<DataVector, Dim>>(outermost_evolved_vars);
            const auto& outermost_d_conformal_metric = get<
                ::Tags::deriv<::Ccz4::Tags::ConformalMetric<DataVector, Dim>,
                              tmpl::size_t<Dim>, Frame::Inertial>>(
                outermost_Ccz4_derivs);
            ...
            const auto& outermost_d_shift =
                get<::Tags::deriv<gr::Tags::Shift<DataVector, Dim>,
                                  tmpl::size_t<Dim>, Frame::Inertial>>(
                    outermost_Ccz4_derivs);
            const auto& outermost_d_b = get<
                ::Tags::deriv<::Ccz4::Tags::AuxiliaryShiftB<DataVector, Dim>,
                              tmpl::size_t<Dim>, Frame::Inertial>>(
                outermost_Ccz4_derivs);
            ...
            ::tenex::evaluate<ti::k, ti::i, ti::j>(
                make_not_null(&outermost_dt_d_conformal_spatial_metric),
                -2.0 * (outermost_d_a_tilde(ti::k, ti::i, ti::j) *
                            outermost_lapse() +
                        outermost_a_tilde(ti::i, ti::j) *
                            outermost_d_lapse(ti::k)) +
                    outermost_d_conformal_metric(ti::k, ti::l, ti::i) *
                        outermost_d_shift(ti::j, ti::L) +
                    outermost_d_conformal_metric(ti::k, ti::l, ti::j) *
                        outermost_d_shift(ti::i, ti::L) +
                    outermost_conformal_metric(ti::i, ti::l) *
                        outermost_d_d_shift(ti::k, ti::j, ti::L) +
                    outermost_conformal_metric(ti::j, ti::l) *
                        outermost_d_d_shift(ti::k, ti::i, ti::L) -
                    2.0 * one_third *
                        (outermost_conformal_metric(ti::i, ti::j) *
                             outermost_d_d_shift(ti::k, ti::l, ti::L) +
                         outermost_d_shift(ti::l, ti::L) *
                             outermost_d_conformal_metric(ti::k, ti::i,
                                                          ti::j)) +
                    outermost_shift(ti::L) * outermost_d_d_conformal_metric(
                                                 ti::k, ti::l, ti::i, ti::j) +
                    outermost_d_shift(ti::k, ti::L) *
                        outermost_d_conformal_metric(ti::l, ti::i, ti::j));

            ::tenex::evaluate<ti::k>(
                make_not_null(&outermost_dt_d_conformal_factor),
                one_third *
                        (outermost_d_trace_extrinsic_curvature(ti::k) *
                             outermost_conformal_factor() * outermost_lapse() +
                         outermost_trace_extrinsic_curvature() *
                             outermost_d_conformal_factor(ti::k) *
                             outermost_lapse() +
                         outermost_trace_extrinsic_curvature() *
                             outermost_d_lapse(ti::k) *
                             outermost_conformal_factor() -
                         outermost_conformal_factor() *
                             outermost_d_d_shift(ti::k, ti::l, ti::L) -
                         outermost_d_conformal_factor(ti::k) *
                             outermost_d_shift(ti::l, ti::L)) +
                    outermost_shift(ti::L) *
                        outermost_d_d_conformal_factor(ti::k, ti::l) +
                    outermost_d_shift(ti::k, ti::L) *
                        outermost_d_conformal_factor(ti::l));

            const auto& k_minus_k0_minus_2_theta_c =
                get<::Ccz4::Tags::KMinusK0Minus2ThetaC<DataVector>>(temp_vars);
            Scalar<DataVector> outermost_k_minus_k0_minus_2_theta_c;
            make_const_view<DataVector>(
                make_not_null(&get(outermost_k_minus_k0_minus_2_theta_c)),
                get(k_minus_k0_minus_2_theta_c),
                (subcell_mesh.extents(2) - 1) * num_face_pts, num_face_pts);
            // We assume in the following d_k_0 = 0.
            ::tenex::evaluate<ti::k>(
                make_not_null(&outermost_dt_d_lapse),
                -2.0 * (outermost_d_lapse(ti::k) *
                            outermost_k_minus_k0_minus_2_theta_c() +
                        outermost_d_trace_extrinsic_curvature(ti::k) *
                            outermost_lapse() -
                        2.0 * outermost_d_theta(ti::k) * c *
                            outermost_lapse()) +
                    outermost_d_shift(ti::k, ti::L) * outermost_d_lapse(ti::l) +
                    outermost_shift(ti::L) * outermost_d_d_lapse(ti::k, ti::l));

            ::tenex::evaluate<ti::k, ti::I>(
                make_not_null(&outermost_dt_d_shift),
                f * outermost_d_b(ti::k, ti::I));
            if (shifting_shift) {
              ::tenex::update<ti::k, ti::I>(
                  make_not_null(&outermost_dt_d_shift),
                  outermost_dt_d_shift(ti::k, ti::I) +
                      outermost_d_shift(ti::k, ti::L) *
                          outermost_d_shift(ti::l, ti::I) +
                      outermost_shift(ti::L) *
                          outermost_d_d_shift(ti::k, ti::l, ti::I));
            }
            ...
            auto dt_char_fields = dt_characteristic_fields(
                ...
                outermost_dt_auxiliary_shift_b,
                outermost_dt_d_conformal_spatial_metric,
            ...
            for (size_t i = 0; i < num_face_pts; ++i) {
              // We require asymptotically Minkowski coordinates
              if (char_speeds[0][i] < 0.0 or char_speeds[1][i] >= 0.0 or
                ...
                ERROR(
                    "ConstraintRadiationPreserving BCs require asymptotically "
                    "Minkowskian"
                    " coordinates but the characteristic speeds at a outer "
                    "boundary point"
                    " are char_speeds[0] = "
                    << char_speeds[0][i]
            ...
            for (size_t i = 0; i < Dim; ++i) {
              make_const_view<DataVector>(
                  make_not_null(&outermost_spatial_z4_constraint_up.get(i)),
                  spatial_z4_constraint_up.get(i),
                  (subcell_mesh.extents(2) - 1) * num_face_pts, num_face_pts);
            }
            const auto constraint_char_fields =
                constraint_characteristic_fields(
                    outermost_theta, outermost_spatial_z4_constraint_up,
                    outermost_spatial_metric, unit_normal_one_form);
            ...
            get(penalty_strength) = penalty_multiplier * abs(char_speeds[4]) /
                                    get(outermost_radial_grid_spacing);

            // Modify the evolution of UVector2Minus and UScalar2Minus to
            // impose constraint preserving BCs. We use the fact below that
            // CScalarMinus has speed -\alpha-\beta^n and CVectorZero has speed
            // -\beta^n.
            auto& dt_u_scalar2_minus =
                get<::Tags::dt<Tags::UScalar2Minus<DataVector>>>(
                    dt_char_fields);
            ::tenex::evaluate(make_not_null(&dt_u_scalar2_minus),
                              -2.0 * penalty_strength() *
                                  outermost_conformal_factor_squared() *
                                  c_scalar_minus());
            auto& dt_u_vector2_minus = get<::Tags::dt<
                Tags::UVector2Minus<DataVector, Dim, Frame::Inertial>>>(
                dt_char_fields);
            ::tenex::evaluate<ti::i>(
                make_not_null(&dt_u_vector2_minus),
                -4.0 * penalty_strength() /
                    (outermost_conformal_factor_squared()) *
                    c_vector_zero(ti::i));
            const Scalar<DataVector> outermost_shift_n =
                dot_product(outermost_shift, unit_normal_one_form);
            for (size_t i = 0; i < num_face_pts; ++i) {
              if (-get(outermost_shift_n)[i] >= 0.0) {
                // c_vector_zero are outgoing at these points; do not add
                // penalty but simply set dt_u_vector2_minus to zero
                for (size_t j = 0; j < Dim; ++j) {
            ...
            const auto& modified_dt_dn_lapse =
                get<::Tags::dt<Tags::DnLapse<DataVector>>>(
                    modified_dt_evolved_vars);
            const auto& modified_dt_dn_shift = get<
                ::Tags::dt<Tags::DnShift<DataVector, Dim, Frame::Inertial>>>(
                modified_dt_evolved_vars);

            // Solve for evolution of conformal_metric, conformal_factor,
            // lapse, and shift using the evolution for their normal derivatives
            // and an interior stencil.
            tnsr::Ij<DataVector, Dim> outermost_inv_jacobian;
            for (size_t tensor_index = 0;
                 tensor_index < outermost_inv_jacobian.size(); ++tensor_index) {
              make_const_view<DataVector>(
                  make_not_null(&outermost_inv_jacobian[tensor_index]),
                  cell_centered_logical_to_inertial_inv_jacobian[tensor_index],
                  (subcell_mesh.extents(2) - 1) * num_face_pts, num_face_pts);
            }
            const tnsr::I<DataVector, Dim> inv_jacobian_dot_normal =
                ::tenex::evaluate<ti::I>(unit_normal_vector(ti::J) *
                                         outermost_inv_jacobian(ti::I, ti::j));
            Scalar<DataVector> jacobian_factor;
            get(jacobian_factor) = inv_jacobian_dot_normal.get(2);
            const double logical_cell_size = 2.0 / subcell_mesh.extents(2);

            const auto reconstruct_dt_tensor =
                [&jacobian_factor, logical_cell_size]<typename TensorType>(
                    const gsl::not_null<TensorType*> outermost_dt_tensor,
                    const std::array<TensorType, 4>& inner_dt_tensors,
                    const TensorType& modified_dt_dn_tensor) {
                  // The following assumes that the \zeta direction lines up
                  // with the normal (radial) direction, which is true for
                  // an inner cube with equidistant coordinates.
                  for (size_t tensor_index = 0;
                       tensor_index < outermost_dt_tensor->size();
                       ++tensor_index) {
                    (*outermost_dt_tensor)[tensor_index] =
                        (12.0 * logical_cell_size *
                             modified_dt_dn_tensor[tensor_index] /
                             get(jacobian_factor) +
                         48.0 * inner_dt_tensors[0][tensor_index] -
                         36.0 * inner_dt_tensors[1][tensor_index] +
                         16.0 * inner_dt_tensors[2][tensor_index] -
                         3.0 * inner_dt_tensors[3][tensor_index]) /
                        25.0;
                  }
                };

            std::array<tnsr::ii<DataVector, Dim>, 4> inner_dt_conformal_metrics;
            for (size_t layer = 0; layer < 4; ++layer) {
              for (size_t tensor_index = 0;
                ...
                make_const_view<DataVector>(
                    make_not_null(
                        &inner_dt_conformal_metrics[layer][tensor_index]),
                    get<::Tags::dt<
                        ::Ccz4::Tags::ConformalMetric<DataVector, Dim>>>(
                        *dt_vars_ptr)[tensor_index],
                    (subcell_mesh.extents(2) - 1 - (layer + 1)) * num_face_pts,
                    num_face_pts);
              }
            }
            auto& outermost_dt_conformal_metric =
                get<::Tags::dt<::Ccz4::Tags::ConformalMetric<DataVector, Dim>>>(
                    outermost_dt_vars);
            reconstruct_dt_tensor(make_not_null(&outermost_dt_conformal_metric),
                                  inner_dt_conformal_metrics,
                                  modified_dt_dn_conformal_metric);

            std::array<Scalar<DataVector>, 4> inner_dt_conformal_factors;
            for (size_t layer = 0; layer < 4; ++layer) {
              make_const_view<DataVector>(
                  make_not_null(&get(inner_dt_conformal_factors[layer])),
                  get(get<
                      ::Tags::dt<::Ccz4::Tags::ConformalFactor<DataVector>>>(
                      *dt_vars_ptr)),
                  (subcell_mesh.extents(2) - 1 - (layer + 1)) * num_face_pts,
                  num_face_pts);
            }
            auto& outermost_dt_conformal_factor =
                get<::Tags::dt<::Ccz4::Tags::ConformalFactor<DataVector>>>(
                    outermost_dt_vars);
            reconstruct_dt_tensor(make_not_null(&outermost_dt_conformal_factor),
                                  inner_dt_conformal_factors,
                                  modified_dt_dn_conformal_factor);

            std::array<Scalar<DataVector>, 4> inner_dt_lapses;
            for (size_t layer = 0; layer < 4; ++layer) {
              make_const_view<DataVector>(
                  make_not_null(&get(inner_dt_lapses[layer])),
                  get(get<::Tags::dt<gr::Tags::Lapse<DataVector>>>(
                      *dt_vars_ptr)),
                  (subcell_mesh.extents(2) - 1 - (layer + 1)) * num_face_pts,
                  num_face_pts);
            }
            auto& outermost_dt_lapse =
                get<::Tags::dt<gr::Tags::Lapse<DataVector>>>(outermost_dt_vars);
            reconstruct_dt_tensor(make_not_null(&outermost_dt_lapse),
                                  inner_dt_lapses, modified_dt_dn_lapse);

            std::array<tnsr::I<DataVector, Dim>, 4> inner_dt_shifts;
            for (size_t layer = 0; layer < 4; ++layer) {
              for (size_t tensor_index = 0;
                   tensor_index < outermost_shift.size(); ++tensor_index) {
                make_const_view<DataVector>(
                    make_not_null(&inner_dt_shifts[layer][tensor_index]),
                    get<::Tags::dt<gr::Tags::Shift<DataVector, Dim>>>(
                        *dt_vars_ptr)[tensor_index],
                    (subcell_mesh.extents(2) - 1 - (layer + 1)) * num_face_pts,
                    num_face_pts);
              }
            }
            auto& outermost_dt_shift =
                get<::Tags::dt<gr::Tags::Shift<DataVector, Dim>>>(
                    outermost_dt_vars);
            reconstruct_dt_tensor(make_not_null(&outermost_dt_shift),
                                  inner_dt_shifts, modified_dt_dn_shift);
          },
          box,
          db::get<
              evolution::dg::subcell::Tags::Coordinates<Dim, Frame::Inertial>>(
          ...
          db::get<evolution::dg::subcell::fd::Tags::
                      InverseJacobianLogicalToInertial<Dim>>(*box),
          db::get<::Ccz4::Tags::SpatialZ4ConstraintUp<DataVector, Dim>>(*box));
    }
  }
};
}  // namespace Ccz4::fd
