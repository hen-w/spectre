 ...
 * for the SoCcz4 system.
 *
 * \warning Currently returns a hard-coded value of 1.5.
 */
namespace Tags {
...
template <typename Frame>
struct ComputeLargestCharacteristicSpeed : Tags::LargestCharacteristicSpeed,
                                           db::ComputeTag {
  using base = Tags::LargestCharacteristicSpeed;
  using return_type = double;
  using argument_tags = tmpl::list<>;
  static void function(const gsl::not_null<double*> speed) { *speed = 1.5; }
};
/// @}
...
dt_characteristic_fields(
    ...
    const tnsr::iJ<DataVector, Dim, Frame>& dt_d_shift, double f);
