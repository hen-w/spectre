 ...
 * with CRPBC (Constraints and Radiation Preserving Boundary Condition) values
 * for the LDG (DG) discretization.
 *
 * \details This MutateApply action runs after
 * `ApplyBoundaryCorrectionsToTimeDerivative` in the DG step. It:
 * 1. Recomputes partial derivatives via spectral differentiation
 * 2. Performs the full CRPBC characteristic decomposition
 *    (matching `SoTimeDerivative.hpp` lines 1040–1728)
 * 3. Overwrites dt at ALL external boundary face nodes
 *
 * \warning Assumes Gauss-Lobatto quadrature (boundary nodes exist).
 * CRPBC can be applied on any external face (potentially multiple
 * simultaneously).
 */
struct OverwriteExternalBoundaryDt {
  ...
  static void apply(
    ...
    ASSERT(mesh.quadrature(0) == Spectral::Quadrature::GaussLobatto and
           ...
           "OverwriteExternalBoundaryDt requires Gauss-Lobatto quadrature "
           "but got "
    ...
    // Loop over each CRPBC face
    for (const auto& direction : crpbc_directions) {
      // Compute face geometry parameters from the direction
      ...
      // Slice helpers: gather a volume component to a contiguous face buffer.
      ...
      //   d_conformal_metric_{ijk} = 2 * field_d_{ijk}
      //   d_conformal_factor_i     = phi * field_p_i
      //   d_lapse_i                = alpha * field_a_i
      //   d_shift_{iJ}             = field_b_{iJ}
      ...
      // Phase D: Compute needed temporaries at boundary face
      ...
      // eq 23: contracted_conformal_christoffel
      ...
      ::tenex::evaluate(
          make_not_null(&k_minus_k0_minus_2_theta_c_face),
          trace_K_face() - 2.0 * c_param * theta_face() - k_0_face());
      ...
      // Phase F: CRPBC characteristic decomposition

      // Get outermost dt values (sliced from dt_vars)
      ...
      // F1: Compute unit_normal_vector and unit_normal_one_form
      // Use inverse Jacobian to compute the outward normal covector
      InverseJacobian<DataVector, Dim, Frame::ElementLogical, Frame::Inertial>
          outermost_inv_jacobian;
      slice_tensor(outermost_inv_jacobian, inv_jacobian);

      // Unnormalized outward normal one-form from inverse Jacobian
      ...
      // Raise to get unnormalized normal vector using inverse spatial metric
      ...
      for (size_t i = 0; i < num_face_pts; ++i) {
        if (char_speeds[0][i] < 0.0 or char_speeds[1][i] >= 0.0 or
          ...
          ERROR(
              ...
              "Minkowskian"
              " coordinates but the characteristic speeds at an outer "
              "boundary point"
              " are char_speeds[0] = "
      ...
      // F6: Zero incoming gauge/shift modes (Sommerfeld-like)
      auto& dt_u_vector3_minus = get<
          ::Tags::dt<Tags::UVector3Minus<DataVector, Dim, Frame::Inertial>>>(
          dt_char_fields);
      ::tenex::evaluate<ti::i>(make_not_null(&dt_u_vector3_minus),
                               0.0 * unit_normal_one_form(ti::i));
      ...
      // F7: Constraint-preserving boundary conditions
      // (ConstraintPreservingMath.tex)

      // Transverse projector q^I_j = δ^I_j - n^I n_j
      const auto q_mixed = gr::transverse_projection_operator(
          unit_normal_vector, unit_normal_one_form);

      // Theta normal derivative: dn_theta = n^i ∂_i θ
      ...
      // Access characteristic field dt components
      ...
      // ---- Equation 1: dt UScalar3Minus ----
      ::tenex::evaluate(
          make_not_null(&dt_u_scalar3_minus),
          dt_u_scalar3_plus() +
              4.0 / conformal_factor_squared_face() *
                  (-lapse_face() * dn_theta() + beta_dot_d_theta()));

      // ---- Equation 2: dt UVector2Minus ----
      // Term 2: (2/φ²) q_i^j γ̃^{kl} q^m_l ∂_t∂_m γ̃_{jk}
      ...
      // Term 3: (4/φ²) q^m_i (-α n^p ∂_p Z_m + β^k ∂_k Z_m)
      ...
      // Term B: φ⁴ n_i q^m_l γ̃^{ij} γ̃^{kl} ∂_t∂_m γ̃_{jk}
      ...
      // ∂_n Z_n = n^p n^m ∂_p Z_m, β·∂Z_n = β^k n^m ∂_k Z_m
      ...
      /* debugging: zero all dt_char_fields */
      // const DataVector used_for_size(num_face_pts);
      // get<::Tags::dt<Tags::UTensorPlus<DataVector, Dim, Frame::Inertial>>>(
      //     dt_char_fields) =
      //     make_with_value<tnsr::ii<DataVector, Dim>>(used_for_size, 0.0);
      // get<::Tags::dt<Tags::UTensorMinus<DataVector, Dim, Frame::Inertial>>>(
      //     dt_char_fields) =
      //     make_with_value<tnsr::ii<DataVector, Dim>>(used_for_size, 0.0);
      // get<::Tags::dt<Tags::UVector1Zero<DataVector, Dim, Frame::Inertial>>>(
      //     dt_char_fields) =
      ...
      //     dt_char_fields) =
      //     make_with_value<tnsr::i<DataVector, Dim>>(used_for_size, 0.0);
      //  get<::Tags::dt<Tags::UVector3Plus<DataVector, Dim, Frame::Inertial>>>(
      //     dt_char_fields) =
      //     make_with_value<tnsr::i<DataVector, Dim>>(used_for_size, 0.0);
      // get<::Tags::dt<Tags::UVector3Minus<DataVector, Dim, Frame::Inertial>>>(
      //     dt_char_fields) =
      //     make_with_value<tnsr::i<DataVector, Dim>>(used_for_size, 0.0);
      // get(get<::Tags::dt<Tags::UScalar1Zero<DataVector>>>(dt_char_fields)) =
      //     DataVector(num_face_pts, 0.0);
      // get(get<::Tags::dt<Tags::UScalar2Plus<DataVector>>>(dt_char_fields)) =
      //     DataVector(num_face_pts, 0.0);
      // get(get<::Tags::dt<Tags::UScalar2Minus<DataVector>>>(dt_char_fields)) =
      //     DataVector(num_face_pts, 0.0);
      //  get(get<::Tags::dt<Tags::UScalar3Plus<DataVector>>>(dt_char_fields)) =
      //     DataVector(num_face_pts, 0.0);
      // get(get<::Tags::dt<Tags::UScalar3Minus<DataVector>>>(dt_char_fields)) =
      //     DataVector(num_face_pts, 0.0);
      // get(get<::Tags::dt<Tags::UScalar4Plus<DataVector>>>(dt_char_fields)) =
      //     DataVector(num_face_pts, 0.0);
      // get(get<::Tags::dt<Tags::UScalar4Minus<DataVector>>>(dt_char_fields)) =
      //     DataVector(num_face_pts, 0.0);
      // get(get<::Tags::dt<Tags::UScalar5Plus<DataVector>>>(dt_char_fields)) =
      //     DataVector(num_face_pts, 0.0);
      // get(get<::Tags::dt<Tags::UScalar5Minus<DataVector>>>(dt_char_fields)) =
      //     DataVector(num_face_pts, 0.0);
      ...
      // Phase G: Overwrite dt at boundary face — directly modified variables
      ...
      // Phase H: Reconstruct dt for metric/gauge vars using spectral
      ...
      // Reconstruct dt_conformal_metric
      {
        ...
        for (size_t ti = 0; ti < dt_cm_vol.size(); ++ti) {
          reconstruct_dt_component(dt_cm_vol[ti].data(),
