...
/*!
 * \brief Experiment 2 variant of the constraints and radiation preserving
 * boundary conditions.
 *
 * Identical to ConstraintsRadiationPreserving (the boundary-integrated
 * conformal metric is still time-integrated by dg_time_derivative), but adds a
 * penalty (relaxation) term driving the boundary-integrated conformal metric
 * toward the interior face values of the evolved conformal metric on the
 * user-specified timescale PenaltyTimescale. Only the conformal metric is
 * affected; the boundary-integrated conformal factor, lapse, and shift are
 * unchanged.
 */
class CRPBCExp2 final : public BoundaryCondition {
 public:
  ...
  /// \brief Timescale of the penalty term relaxing the boundary-integrated
  /// conformal metric toward the interior face values.
  struct PenaltyTimescale {
    static constexpr Options::String help =
        "Timescale (in units of M) of the penalty term driving the "
        "boundary-integrated conformal metric toward the interior face values "
        "of the evolved conformal metric.";
    using type = double;
  };
  ...
  static constexpr Options::String help{
      "Experiment 2 variant of the constraints and radiation preserving "
      "boundary conditions. Identical to ConstraintsRadiationPreserving but "
      "adds a penalty term relaxing the boundary-integrated conformal metric "
      "toward the interior face values on the timescale PenaltyTimescale."};

  CRPBCExp2() = default;
  CRPBCExp2(CRPBCExp2&&) = default;
  CRPBCExp2& operator=(CRPBCExp2&&) =
      default;
  CRPBCExp2(const CRPBCExp2&);
  CRPBCExp2& operator=(
      const CRPBCExp2&);
  ~CRPBCExp2() override = default;

  explicit CRPBCExp2(CkMigrateMessage* msg);
