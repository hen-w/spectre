...
CRPBCExp1::get_clone() const {
  return std::make_unique<CRPBCExp1>(*this);
}

void CRPBCExp1::pup(PUP::er& p) {
...
// NOLINTNEXTLINE
PUP::able::PUP_ID CRPBCExp1::my_PUP_ID = 0;

void CRPBCExp1::apply_ghost_ylm_filter_impl(
    const gsl::not_null<tnsr::ii<DataVector, 3, Frame::Inertial>*>
        conformal_metric,
...
std::optional<std::string> CRPBCExp1::dg_ghost(
    const gsl::not_null<tnsr::ii<DataVector, 3, Frame::Inertial>*>
        conformal_metric,
    const gsl::not_null<Scalar<DataVector>*> conformal_factor,
    const gsl::not_null<tnsr::ii<DataVector, 3, Frame::Inertial>*> a_tilde,
  ...
  ASSERT(evolve_lapse_and_shift,
         "CRPBCExp1 BC is not implemented"
  ...
  for (size_t s = 0; s < num_pts; ++s) {
    if (result.char_speeds[0][s] < 0.0 || result.char_speeds[1][s] >= 0.0 ||
      ...
      ERROR(
          "CRPBCExp1 dg_ghost: characteristic speed sign "
  ...
  // Ghost conformal factor, lapse, and shift = boundary-integrated; ghost
  // conformal metric = interior face value (Experiment 1)
  ...
  *field_p = std::move(result.field_p);
  ...
  // Enforce g_tilde^{jk} D_{ijk} = 0 w.r.t. the interior conformal metric
  // (Jacobi formula for det(g_tilde) = 1)
  {
    ...
    ::tenex::update<ti::k, ti::i, ti::j>(
        ...
        (*field_d)(ti::k, ti::i, ti::j) -
            residual(ti::k) *
                interior_conformal_metric(ti::i, ti::j) / 3.0);
  }
...
std::optional<std::string> CRPBCExp1::dg_time_derivative(
    const gsl::not_null<tnsr::ii<DataVector, 3, Frame::Inertial>*>
...
[[noreturn]] void CRPBCExp1::fd_ghost(
    const gsl::not_null<tnsr::ii<DataVector, 3, Frame::Inertial>*>,
    const gsl::not_null<Scalar<DataVector>*>,
    const gsl::not_null<tnsr::I<DataVector, 3, Frame::Inertial>*>,
    const gsl::not_null<Scalar<DataVector>*>,
  ...
  ERROR(
      "CRPBCExp1 fd_ghost is not implemented. "
