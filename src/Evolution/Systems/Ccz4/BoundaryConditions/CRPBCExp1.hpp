...
/*!
 * \brief Experiment 1 variant of ConstraintsRadiationPreserving: the ghost
 * conformal metric is the interior face value rather than the time-integrated
 * boundary field.
 *
 * Identical to ConstraintsRadiationPreserving except that the ghost (exterior)
 * conformal metric handed to the boundary correction is copied from the
 * interior face value of the evolved conformal metric (exact on the
 * Gauss-Lobatto grids used here) instead of the boundary-integrated
 * accumulator. Only the conformal metric changes; the boundary-integrated
 * conformal factor, lapse, and shift are untouched, and the accumulator keeps
 * time-integrating as a diagnostic.
 */
class CRPBCExp1 final : public BoundaryCondition {
 public:
  ...
  static constexpr Options::String help{
      "Experiment 1 variant of ConstraintsRadiationPreserving. Identical to "
      "it except that the ghost conformal metric is copied from the interior "
      "face value of the evolved conformal metric instead of the "
      "time-integrated boundary field; the boundary-integrated conformal "
      "factor, lapse, and shift are unchanged."};

  CRPBCExp1() = default;
  CRPBCExp1(CRPBCExp1&&) = default;
  CRPBCExp1& operator=(CRPBCExp1&&) =
      default;
  CRPBCExp1(const CRPBCExp1&);
  CRPBCExp1& operator=(
      const CRPBCExp1&);
  ~CRPBCExp1() override = default;

  explicit CRPBCExp1(CkMigrateMessage* msg);
