...
CRPBCExp2::get_clone() const {
  return std::make_unique<CRPBCExp2>(*this);
}

void CRPBCExp2::pup(PUP::er& p) {
...
// NOLINTNEXTLINE
PUP::able::PUP_ID CRPBCExp2::my_PUP_ID = 0;

void CRPBCExp2::apply_ghost_ylm_filter_impl(
    const gsl::not_null<tnsr::ii<DataVector, 3, Frame::Inertial>*>
        conformal_metric,
...
std::optional<std::string> CRPBCExp2::dg_ghost(
    const gsl::not_null<tnsr::ii<DataVector, 3, Frame::Inertial>*>
        conformal_metric,
    const gsl::not_null<Scalar<DataVector>*> conformal_factor,
    const gsl::not_null<tnsr::ii<DataVector, 3, Frame::Inertial>*> a_tilde,
  ...
  ASSERT(evolve_lapse_and_shift,
         "CRPBCExp2 BC is not implemented"
  ...
  for (size_t s = 0; s < num_pts; ++s) {
    if (result.char_speeds[0][s] < 0.0 || result.char_speeds[1][s] >= 0.0 ||
      ...
      ERROR(
          "CRPBCExp2 dg_ghost: characteristic speed sign "
...
std::optional<std::string> CRPBCExp2::dg_time_derivative(
    const gsl::not_null<tnsr::ii<DataVector, 3, Frame::Inertial>*>
  ...
  // interior face values on timescale penalty_timescale_.
  ::tenex::update<ti::i, ti::j>(
      dt_boundary_conformal_metric_correction,
      (*dt_boundary_conformal_metric_correction)(ti::i, ti::j) -
          (interior_boundary_conformal_metric(ti::i, ti::j) -
           interior_conformal_metric(ti::i, ti::j)) /
              penalty_timescale_);

  return {};
}

[[noreturn]] void CRPBCExp2::fd_ghost(
    const gsl::not_null<tnsr::ii<DataVector, 3, Frame::Inertial>*>,
    const gsl::not_null<Scalar<DataVector>*>,
    const gsl::not_null<tnsr::I<DataVector, 3, Frame::Inertial>*>,
    const gsl::not_null<Scalar<DataVector>*>,
  ...
  ERROR(
      "CRPBCExp2 fd_ghost is not implemented. "
