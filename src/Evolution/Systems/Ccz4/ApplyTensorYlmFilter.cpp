...
void transform_ghost_tensors_to_different_frame(
    const gsl::not_null<Variables<ccz4_ghost_vars_list<DestFrame>>*> dest,
    const Variables<ccz4_ghost_vars_list<SrcFrame>>& src,
    const InverseJacobian<DataVector, 3, SrcFrame, DestFrame>& jac_for_lower,
    const InverseJacobian<DataVector, 3, DestFrame, SrcFrame>&
        inv_jac_for_upper) {
  const auto& [src_conf_metric, src_conf_factor, src_a_tilde, src_trace_k,
               src_theta, src_gamma_hat, src_lapse, src_shift, src_aux_shift,
               src_field_a, src_field_b, src_field_d, src_field_p] = src;
  auto& [dest_conf_metric, dest_conf_factor, dest_a_tilde, dest_trace_k,
         dest_theta, dest_gamma_hat, dest_lapse, dest_shift, dest_aux_shift,
         dest_field_a, dest_field_b, dest_field_d, dest_field_p] = *dest;
  ...
  // Transform tnsr::i variables (FieldA, FieldP): dest_i = J^k_i * src_k
  for (size_t i = 0; i < 3; ++i) {
    dest_field_a.get(i) =
        jac_for_lower.get(0, i) * src_field_a.get(0) +
        jac_for_lower.get(1, i) * src_field_a.get(1) +
        jac_for_lower.get(2, i) * src_field_a.get(2);
    dest_field_p.get(i) =
        jac_for_lower.get(0, i) * src_field_p.get(0) +
        jac_for_lower.get(1, i) * src_field_p.get(1) +
        jac_for_lower.get(2, i) * src_field_p.get(2);
  }

  // Transform tnsr::iJ (FieldB): dest_i^J = J^k_i * (J^{-1})^J_l * src_k^l
  for (size_t i = 0; i < 3; ++i) {
    for (size_t j = 0; j < 3; ++j) {
      dest_field_b.get(i, j) = 0.0;
      for (size_t k = 0; k < 3; ++k) {
        for (size_t l = 0; l < 3; ++l) {
          dest_field_b.get(i, j) += jac_for_lower.get(k, i) *
                                    inv_jac_for_upper.get(j, l) *
                                    src_field_b.get(k, l);
        }
      }
    }
  ...
  // Transform tnsr::ijj (FieldD): dest_{ijk} = J^l_i * J^m_j * J^n_k *
  // src_{lmn}
  for (size_t i = 0; i < 3; ++i) {
    for (size_t j = 0; j < 3; ++j) {
      for (size_t k = j; k < 3; ++k) {
        dest_field_d.get(i, j, k) = 0.0;
        for (size_t l = 0; l < 3; ++l) {
          for (size_t m = 0; m < 3; ++m) {
            for (size_t n = 0; n < 3; ++n) {
              dest_field_d.get(i, j, k) += jac_for_lower.get(l, i) *
                                           jac_for_lower.get(m, j) *
                                           jac_for_lower.get(n, k) *
                                           src_field_d.get(l, m, n);
            }
          }
        }
      }
...
void apply_tensor_ylm_filter_ghost(
  ...
  ASSERT(
      radial_extents * ylm.physical_size() ==
          ghost_vars->number_of_grid_points(),
  ...
  // Memory aliasing pattern (same as apply_tensor_ylm_filter):
  Variables<filter_detail::ccz4_ghost_vars_list<Frame::Grid>>
      ghost_spectral_vars(temp_storage->data(), temp_storage->size());
  Variables<filter_detail::ccz4_ghost_vars_list<Frame::Grid>> temp_grid_vars(
      ghost_vars->data(), ghost_vars->size());
  ASSERT(ghost_vars->size() <= temp_storage->size(),
         "Should have " << ghost_vars->size() << " <= "
                        << temp_storage->size());
  Variables<filter_detail::ccz4_ghost_vars_list<Frame::Grid>> temp_ghost_vars(
      temp_storage->data(), ghost_vars->size());

  // 1. Inertial → Grid frame transform
  filter_detail::transform_ghost_tensors_to_different_frame<Frame::Inertial,
                                                            Frame::Grid>(
      make_not_null(&temp_ghost_vars), *ghost_vars, jac_inertial_to_grid,
      jac_grid_to_inertial);

  // 1a. Copy
  std::memcpy(temp_grid_vars.data(), temp_ghost_vars.data(),
              temp_ghost_vars.size() * sizeof(double));

  // 2. Nodal → Modal
  ylm::TensorYlm::filter_detail::nodal_to_modal_ylm(
      make_not_null(&ghost_spectral_vars), temp_grid_vars, ylm,
      radial_extents);

  // 3. Filter — dispatch by tensor symmetry (5 branches)
  tmpl::for_each<filter_detail::ccz4_ghost_vars_list<Frame::Grid>>(
      [&ghost_spectral_vars, &temp_grid_vars, radial_extents,
       &filter_matrix_scalar, &filter_matrix_i, &filter_matrix_ii,
       &filter_matrix_ij,
       &filter_matrix_kii]<class Tag>(const tmpl::type_<Tag> /*meta*/) {
        (void)radial_extents;
        constexpr size_t num_independent_components =
            Tag::type::structure::size();
        ASSERT(ghost_spectral_vars.number_of_grid_points() *
                       num_independent_components <=
                   temp_grid_vars.size(),
               "Insufficient size: must have "
                   << ghost_spectral_vars.number_of_grid_points() *
                          num_independent_components
                   << " <= " << temp_grid_vars.size());
        ...
        get<Tag>(dest_tensor) = get<Tag>(ghost_spectral_vars);

        const gsl::span<double> src(
            get<Tag>(ghost_spectral_vars)[0].data(),
            num_independent_components *
                ghost_spectral_vars.number_of_grid_points());
        ...
        get<Tag>(ghost_spectral_vars) = get<Tag>(dest_tensor);
      });

  // 4. Modal → Nodal
  ylm::TensorYlm::filter_detail::modal_to_nodal_ylm(
      make_not_null(&temp_grid_vars), ghost_spectral_vars, ylm,
      radial_extents);

  // 4a. Copy
  std::memcpy(temp_ghost_vars.data(), temp_grid_vars.data(),
              temp_grid_vars.size() * sizeof(double));

  // 5. Grid → Inertial frame transform
  filter_detail::transform_ghost_tensors_to_different_frame<Frame::Grid,
                                                            Frame::Inertial>(
      ghost_vars, temp_ghost_vars, jac_grid_to_inertial, jac_inertial_to_grid);
}
...
template void transform_spatial_tensors_to_different_frame_without_hessians<
    ...
    const InverseJacobian<DataVector, 3, Frame::Grid, Frame::Inertial>&
        inv_jac_for_upper);
// Ghost vars frame transform instantiations
template void transform_ghost_tensors_to_different_frame<Frame::Grid,
                                                         Frame::Inertial>(
    gsl::not_null<Variables<ccz4_ghost_vars_list<Frame::Inertial>>*> dest,
    const Variables<ccz4_ghost_vars_list<Frame::Grid>>& src,
    const InverseJacobian<DataVector, 3, Frame::Grid, Frame::Inertial>&
        jac_for_lower,
    const InverseJacobian<DataVector, 3, Frame::Inertial, Frame::Grid>&
        inv_jac_for_upper);

template void transform_ghost_tensors_to_different_frame<Frame::Inertial,
                                                         Frame::Grid>(
    gsl::not_null<Variables<ccz4_ghost_vars_list<Frame::Grid>>*> dest,
    const Variables<ccz4_ghost_vars_list<Frame::Inertial>>& src,
    const InverseJacobian<DataVector, 3, Frame::Inertial, Frame::Grid>&
        jac_for_lower,
    const InverseJacobian<DataVector, 3, Frame::Grid, Frame::Inertial>&
        inv_jac_for_upper);
...
YLM_TENSORYLM_INSTANTIATE_MODAL_NODAL_TRANSFORMS(
    Ccz4::filter_detail::ccz4_ghost_vars_list<Frame::Grid>);
YLM_TENSORYLM_INSTANTIATE_MODAL_NODAL_TRANSFORMS(
