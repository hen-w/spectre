...
template <size_t Dim>
ProposedFlux<Dim>::ProposedFlux(CkMigrateMessage* msg)
    : BoundaryCorrection(msg) {}

template <size_t Dim>
ProposedFlux<Dim>::ProposedFlux(const double tau1, const double tau2,
...
template <size_t Dim>
std::unique_ptr<evolution::BoundaryCorrection> ProposedFlux<Dim>::get_clone()
    const {
  return std::make_unique<ProposedFlux>(*this);
}

template <size_t Dim>
void ProposedFlux<Dim>::pup(PUP::er& p) {
...
template <size_t Dim>
double ProposedFlux<Dim>::dg_package_data(
...
void ProposedFlux<Dim>::dg_boundary_terms(
    gsl::not_null<tnsr::ii<DataVector, Dim, Frame::Inertial>*>
  ...
  *boundary_shift_boundary_correction =
  ...
  const double effective_tau1 =
      (ForExternalBoundary and use_central_flux_at_boundary_) ? 0.0 : tau1_;
  const double effective_tau2 =
      (ForExternalBoundary and use_central_flux_at_boundary_) ? 1.0 : tau2_;
  ...
  *shift_boundary_correction =
  ...
  // Compute, on each side separately, the trace T = B_k^k of FieldB (a mixed
  // spatial derivative of the shift) and the trace-free symmetric combination
  // Q_ij = 0.5 * (g_jk B_i^k + g_ik B_j^k) - (1/3) g_ij T, where the upper
  // index of FieldB is lowered with that side's own conformal spatial metric.
  // WARNING: This metric lowering is an unvalidated algebraic extension of
  // the Cartesian Minkowski prescription. Trace-freeness alone does not
  // establish that these penalty arguments are appropriate, or stabilize the
  // system, away from Minkowski; a curved-background analysis is still needed.
  // These modify only the dissipative Lax-Friedrichs jumps of ATilde, K, and
  // Theta below; the central fluxes are untouched.
  Scalar<DataVector> trace_field_b_int;
  ::tenex::evaluate(make_not_null(&trace_field_b_int),
                    field_b_int(ti::k, ti::K));
  Scalar<DataVector> trace_field_b_ext;
  ::tenex::evaluate(make_not_null(&trace_field_b_ext),
                    field_b_ext(ti::k, ti::K));
  tnsr::ii<DataVector, Dim, Frame::Inertial> q_int;
  ::tenex::evaluate<ti::i, ti::j>(
      make_not_null(&q_int),
      0.5 * (conformal_metric_int(ti::j, ti::k) * field_b_int(ti::i, ti::K) +
             conformal_metric_int(ti::i, ti::k) * field_b_int(ti::j, ti::K)) -
          (1.0 / 3.0) * conformal_metric_int(ti::i, ti::j) *
              trace_field_b_int());
  tnsr::ii<DataVector, Dim, Frame::Inertial> q_ext;
  ::tenex::evaluate<ti::i, ti::j>(
      make_not_null(&q_ext),
      0.5 * (conformal_metric_ext(ti::j, ti::k) * field_b_ext(ti::i, ti::K) +
             conformal_metric_ext(ti::i, ti::k) * field_b_ext(ti::j, ti::K)) -
          (1.0 / 3.0) * conformal_metric_ext(ti::i, ti::j) *
              trace_field_b_ext());
  ...
  // Experimental modification: the dissipative jump [K] is replaced by
  // [K - T / alpha], using each side's own lapse and T = B_k^k.
  ::tenex::evaluate(trace_extrinsic_curvature_boundary_correction,
                    -0.5 * effective_tau2 *
                            (trace_extrinsic_curvature_flux_dot_normal_ext() +
                             trace_extrinsic_curvature_flux_dot_normal_int()) -
                        0.5 * effective_tau1 *
                            ((trace_extrinsic_curvature_ext() -
                              trace_field_b_ext() / lapse_ext()) -
                             (trace_extrinsic_curvature_int() -
                              trace_field_b_int() / lapse_int())));
  ...
  // Experimental modification: the dissipative jump [ATilde_ij] is replaced by
  // [ATilde_ij - Q_ij / alpha], using each side's own Q_ij and lapse.
  ::tenex::evaluate<ti::i, ti::j>(
      a_tilde_boundary_correction,
      -0.5 * effective_tau2 *
              (a_tilde_flux_dot_normal_ext(ti::i, ti::j) +
               a_tilde_flux_dot_normal_int(ti::i, ti::j)) -
          0.5 * effective_tau1 *
              ((a_tilde_ext(ti::i, ti::j) - q_ext(ti::i, ti::j) / lapse_ext()) -
               (a_tilde_int(ti::i, ti::j) -
                q_int(ti::i, ti::j) / lapse_int())));
  ...
  // Experimental modification: the dissipative jump [Theta] is replaced by
  // [Theta - T / (2 alpha)], using each side's own lapse and T = B_k^k.
  ::tenex::evaluate(
      theta_boundary_correction,
      -0.5 * effective_tau2 *
              (theta_flux_dot_normal_ext() + theta_flux_dot_normal_int()) -
          0.5 * effective_tau1 *
              ((theta_ext() - 0.5 * trace_field_b_ext() / lapse_ext()) -
               (theta_int() - 0.5 * trace_field_b_int() / lapse_int())));
  ...
  ::tenex::evaluate<ti::I>(
      gamma_hat_boundary_correction,
      -0.5 * effective_tau2 *
              (gamma_hat_flux_dot_normal_ext(ti::I) +
  ...
  ::tenex::evaluate<ti::I>(
      auxiliary_shift_b_boundary_correction,
      -0.5 * effective_tau2 *
              (b_flux_dot_normal_ext(ti::I) + b_flux_dot_normal_int(ti::I)) -
          0.5 * effective_tau1 *
  ...
  ::tenex::evaluate<ti::k>(
      field_a_boundary_correction,
      -0.5 * effective_tau2 *
              (field_a_flux_dot_normal_int(ti::k) +
  ...
  ::tenex::evaluate<ti::k, ti::I>(
      field_b_boundary_correction,
      -0.5 * effective_tau2 *
              (field_b_flux_dot_normal_int(ti::k, ti::I) +
  ...
  ::tenex::evaluate<ti::k, ti::i, ti::j>(
      field_d_boundary_correction,
      -0.5 * effective_tau2 *
              (field_d_flux_dot_normal_int(ti::k, ti::i, ti::j) +
...
template <size_t Dim>
void ProposedFlux<Dim>::dg_auxiliary_boundary_terms(
    gsl::not_null<tnsr::ii<DataVector, Dim, Frame::Inertial>*>
        conformal_metric_boundary_correction,
    gsl::not_null<Scalar<DataVector>*> conformal_factor_boundary_correction,
    gsl::not_null<tnsr::ii<DataVector, Dim, Frame::Inertial>*>
...
template <size_t Dim>
// NOLINTNEXTLINE
PUP::able::PUP_ID ProposedFlux<Dim>::my_PUP_ID = 0;

#define DIM(data) BOOST_PP_TUPLE_ELEM(0, data)

#define INSTANTIATION(_, data)                                                 \
  template class ProposedFlux<DIM(data)>;                                      \
  template void ProposedFlux<DIM(data)>::dg_boundary_terms<false>(             \
      ...
      const tnsr::ijj<DataVector, DIM(data), Frame::Inertial>&,                \
      const tnsr::i<DataVector, DIM(data), Frame::Inertial>&,                  \
      const tnsr::i<DataVector, DIM(data), Frame::Inertial>&, dg::Formulation) \
      const;                                                                   \
  template void ProposedFlux<DIM(data)>::dg_boundary_terms<true>(              \
      gsl::not_null<tnsr::ii<DataVector, DIM(data), Frame::Inertial>*>,        \
      gsl::not_null<Scalar<DataVector>*>,                                      \
      gsl::not_null<tnsr::ii<DataVector, DIM(data), Frame::Inertial>*>,        \
      gsl::not_null<Scalar<DataVector>*>, gsl::not_null<Scalar<DataVector>*>,  \
