...
/*!
 * \brief Computes the auxiliary variable \f$\Phi_i = \partial_i \Psi\f$ from
 * the current evolved \f$\Psi\f$ and stores it back into the evolved
 * variables.
 *
 ...
 * `ApplyAuxiliaryBoundaryCorrectionsToVariables` will fix up \f$\Phi_i\f$
...
struct UpdateAuxiliaryVariables {
  ...
  static void apply(
    ...
    *phi = partial_derivative(psi, mesh, inverse_jacobian);
  }
};
}  // namespace SoScalarWave
