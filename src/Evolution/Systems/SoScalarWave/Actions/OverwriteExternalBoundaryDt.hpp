...
struct OverwriteExternalBoundaryDt {
  ...
  static void apply(
      const gsl::not_null<
          Variables<typename dt_variables_tag::tags_list>*> dt_vars,
    ...
    for (const auto& direction : element.external_boundaries()) {
      ...
      const auto* const td_bc =
          dynamic_cast<
              const SoScalarWave::BoundaryConditions::TimeDerivativeDirichlet<
                  Dim>*>(&bc_base);
      if (td_bc == nullptr) {
        continue;
      }
      ...
      tnsr::i<DataVector, Dim, Frame::Inertial> dt_phi_correction{
          num_face_pts};
      const tnsr::i<DataVector, Dim, Frame::Inertial> unused_normal{
          num_face_pts, 0.0};
      td_bc->dg_time_derivative(
          make_not_null(&dt_psi_correction),
          make_not_null(&dt_pi_correction),
          make_not_null(&dt_phi_correction),
          std::nullopt, unused_normal,
          face_coords,
          get<dt_psi_tag>(current_dt_on_face),
          get<dt_pi_tag>(current_dt_on_face),
          time);
