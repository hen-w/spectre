...
DirichletCharacteristics<Dim>::boundary_field_time_derivatives(
    ...
    [[maybe_unused]] const double time) const {
  if (copy_psi_from_interior_) {
    // The boundary-evolved field is not used; its time derivative is zero.
    get(*dt_boundary_psi) = 0.0;
  } else {
    // Compute Pi_boundary from the characteristic decomposition and set
    ...
    // dt BoundaryPsi = -Pi_boundary
    get(*dt_boundary_psi) = -pi_boundary;
  }
