...
///     declares a non-empty `auxiliary_variables`; allocated but not populated
///     here - an evolution system that declares auxiliary variables must run an
///     action that populates them before they are read)
///
/// - Removes: nothing
/// - Modifies: nothing
...
struct NonconservativeSystem {
  ...
  // A system may declare `auxiliary_variables` (e.g. LDG systems, whose
  // auxiliary variables are recomputed each step rather than time-evolved).
  // Those variables live in their own storage, `::Tags::Variables<
  // auxiliary_variables>`, which we allocate here alongside `variables_tag`.
  // Detect-or-default yields an empty list for systems without the alias, in
  // which case no auxiliary storage is added.
