...
/// size. To fix, this action copies the volume history's
...
struct ChangeSlabSizeBoundaryEvolvedFields {
  ...
  static Parallel::iterable_action_return_t apply(
    ...
    if (volume_history.empty()) {
      for (const auto& [direction, history] : db::get<history_tag>(box)) {
        if (not history.empty()) {
          const auto& failed_direction = direction;
          ERROR(
              "The volume time-stepper history is empty, but the "
              "boundary-evolved-fields history in direction "
              << failed_direction
              << " is not. The histories must be recorded and cleaned in "
                 "lockstep.");
        }
      }
      return {Parallel::AlgorithmExecution::Continue, std::nullopt};
        ...
        [&volume_latest_id](
          ...
          for (auto& [direction, history] : *history_map) {
            if (history.empty()) {
              const auto& failed_direction = direction;
              ERROR(
                  "The volume time-stepper history is not empty, but the "
                  "boundary-evolved-fields history in direction "
                  << failed_direction
                  << " is. The histories must be recorded and cleaned in "
                     "lockstep.");
            }
            if (history.back().time_step_id != volume_latest_id) {
