...
// the axis itself is covered by test_axis_regularity.
template <typename Generator>
std::array<double, 3> point_at_radius(const gsl::not_null<Generator*> generator,
                                      const double radius) {
  std::uniform_real_distribution<> theta_distribution(0.15, M_PI - 0.15);
  std::uniform_real_distribution<> phi_distribution(0., 2. * M_PI);
  const double cos_theta = std::cos(theta_distribution(*generator));
...
void test_pypp_all_shells(const gsl::not_null<Generator*> generator,
  ...
  for (size_t shell = 0; shell < num_shells; ++shell) {
    ...
    const double radius = lower + (upper - lower) * unit(*generator);
  ...
  // double case: one generic outer-sheet point (intermediate shell).
  const double radius_double =
      throat * (3. + 9. * unit(*generator));  // in [3, 12] throat radii
...
// The ADM identity is checked against gr::extrinsic_curvature at 1e-12
...
void test_consistency(const double mass, const double dimensionless_spin) {
  ...
  // Two-sided FD stencil half-width. Points are placed at least a few `delta`
  // away from the throat so no stencil crosses it.
  const double delta = 1.e-4;
  // FD truncation floor ~1.4e-10 (relative); tightest passing decade 1e-9,
  // final one decade looser (truncation-limited, not roundoff).
  const double tolerance = 1.e-8;
  // ADM identity floor ~2.8e-14 (absolute); 30x headroom -> 1e-12.
  ...
  // Near-throat on the outer sheet, at r = 1.5 r_+/4 (|alpha| ~ 0.08-0.19;
  // closer to the throat the 1/alpha amplification grows without bound).
  check_point(
      tnsr::I<double, 3>{{{0.45 * throat, 0.6 * throat, 1.2990381 * throat}}});
  // Generic inner-sheet point, r ~ 0.616 r_+/4.
...
// Item 4: exact Schwarzschild limit chi = 0. In isotropic coordinates the
// puncture radial coordinate r coincides with the isotropic radius, so
//   gamma_ij = (1 + M/2r)^4 delta_ij, alpha = (1 - M/2r) / (1 + M/2r),
//   K_ij = 0, beta^i = 0.
// The lapse is signed (negative for r < M/2), matching the isotropic-
// coordinates lapse of Xcts::Solutions::Schwarzschild.
template <typename DataType>
void test_schwarzschild_limit(const DataType& used_for_size) {
  const double mass = 1.7;
  const HighSpinKerrPuncture solution(mass, 0.);
  const double throat = 0.25 * r_plus_of(mass, 0.);  // = M/2

  // Points on both sheets, off-axis and off-equator.
  DataType r_values{};
  if constexpr (std::is_same_v<DataType, double>) {
    r_values = 2.5 * throat;  // outer sheet
  } else {
    r_values =
        DataType{{0.4 * throat, 0.7 * throat, 1.5 * throat, 4. * throat}};
  }
  auto x =
  ...
  const auto zero_ii = make_with_value<tnsr::ii<DataType, 3, Frame::Inertial>>(
      used_for_size, 0.);
...
void test_trace_k_vanishes(const double mass, const double dimensionless_spin) {
  ...
  Approx trace_approx = Approx::custom().epsilon(1.e-14).scale(1.);
...
void test_inverse_and_determinant(const double mass,
  ...
  for (size_t i = 0; i < 3; ++i) {
    for (size_t j = 0; j < 3; ++j) {
      DataVector product(size_t{4}, 0.);
      for (size_t k = 0; k < 3; ++k) {
        product += spatial_metric.get(i, k) * inverse_metric.get(k, j);
      }
      const DataVector expected_identity(size_t{4}, (i == j) ? 1. : 0.);
...
void test_copy_and_move() {
  ...
  HighSpinKerrPuncture solution_copy(1.5, 0.8);
...
SPECTRE_TEST_CASE(
  ...
  // the extrinsic curvature vanishes identically. The Python reference is
  // regular there (sin(theta) = 1).
  {
  ...
  test_schwarzschild_limit<double>(0.);
  test_schwarzschild_limit<DataVector>(DataVector(size_t{4}));
