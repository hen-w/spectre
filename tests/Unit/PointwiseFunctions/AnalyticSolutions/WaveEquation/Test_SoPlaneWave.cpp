  ...
  struct factory_creation
    ...
    using factory_classes = tmpl::map<
        ...
        tmpl::pair<evolution::initial_data::InitialData,
                   tmpl::list<SoScalarWave::Solutions::SoPlaneWave<Dim>>>>;
  };
};
...
void check_solution(
    ...
    const SoScalarWave::Solutions::SoPlaneWave<DimSolution>& pw,
  ...
  // The full evolution variables {Psi, Pi, Phi}
  const auto vars =
      pw.variables(x, t,
                   tmpl::list<SoScalarWave::Tags::Psi, SoScalarWave::Tags::Pi,
                              SoScalarWave::Tags::Phi<DimSolution>>{});
  CHECK_ITERABLE_APPROX(get<SoScalarWave::Tags::Psi>(vars), pw.psi(x, t));
  CHECK_ITERABLE_APPROX(get<SoScalarWave::Tags::Phi<DimSolution>>(vars),
                        pw.dpsi_dx(x, t));
  CHECK_ITERABLE_APPROX(get<SoScalarWave::Tags::Pi>(vars),
                        Scalar<DataVector>(-1.0 * pw.dpsi_dt(x, t).get()));

  // The LDG evolved variables {Psi, Pi} must match the {Psi, Pi, Phi} overload
  const auto ldg_vars = pw.variables(
      x, t, tmpl::list<SoScalarWave::Tags::Psi, SoScalarWave::Tags::Pi>{});
  CHECK_ITERABLE_APPROX(get<SoScalarWave::Tags::Psi>(ldg_vars),
                        get<SoScalarWave::Tags::Psi>(vars));
  CHECK_ITERABLE_APPROX(get<SoScalarWave::Tags::Pi>(ldg_vars),
                        get<SoScalarWave::Tags::Pi>(vars));

  // The time derivatives of the evolution variables
  const auto dt_vars = pw.variables(
      x, t,
      tmpl::list<Tags::dt<SoScalarWave::Tags::Psi>,
                 Tags::dt<SoScalarWave::Tags::Pi>,
                 Tags::dt<SoScalarWave::Tags::Phi<DimSolution>>>{});
  CHECK_ITERABLE_APPROX(get<Tags::dt<SoScalarWave::Tags::Psi>>(dt_vars),
                        pw.dpsi_dt(x, t));
  CHECK_ITERABLE_APPROX(
      get<Tags::dt<SoScalarWave::Tags::Phi<DimSolution>>>(dt_vars),
      pw.d2psi_dtdx(x, t));
  CHECK_ITERABLE_APPROX(get<Tags::dt<SoScalarWave::Tags::Pi>>(dt_vars),
...
void test_1d() {
  ...
  const SoScalarWave::Solutions::SoPlaneWave<1> pw(
      {{kx}}, {{center_x}},
  ...
  const auto& created_solution =
      dynamic_cast<const SoScalarWave::Solutions::SoPlaneWave<1>&>(
...
void test_2d() {
  ...
  const SoScalarWave::Solutions::SoPlaneWave<2> pw(
  ...
  const auto deserialized_option_solution =
      serialize_and_deserialize(option_solution);
  const auto& created_solution =
      dynamic_cast<const SoScalarWave::Solutions::SoPlaneWave<2>&>(
          *deserialized_option_solution);
...
void test_3d() {
  ...
  const SoScalarWave::Solutions::SoPlaneWave<3> pw(
  ...
  const auto deserialized_option_solution =
      serialize_and_deserialize(option_solution);
  const auto& created_solution =
      dynamic_cast<const SoScalarWave::Solutions::SoPlaneWave<3>&>(
          *deserialized_option_solution);
...
SPECTRE_TEST_CASE(
    "Unit.PointwiseFunctions.AnalyticSolutions.WaveEquation.SoPlaneWave",
