...
// Time stepping of a BoundaryVariables entry of a list-valued
// variables_tag through the standard self-start and step actions; see
// the namespace's members for the covered checks.
namespace boundary_variables_stepping {
        ...
        [&volume, &boundary](
          ...
          for (auto& [direction, dt_variables] : dt_boundary->variables()) {
            get(get<::Tags::dt<Tags_ns::BoundaryValue<VolumeVar>>>(
                dt_variables)) = face_rate(direction) *
...
struct InitializeTimeStepperHistory {
  ...
  static void apply(db::DataBox<DbTagsList>& box,
    ...
    db::mutate_apply<
        Initialization::TimeStepperHistory<typename Metavariables::system>>(
        make_not_null(&box));
  }
};
...
void emplace(const gsl::not_null<MockRuntimeSystem*> runner,
  ...
  ActionTesting::emplace_component_and_initialize<component>(
      ...
      {std::move(volume), boundary_variables, typename dt_volume_tag::type{1},
       ...
       TimeStepId(forward_in_time, -static_cast<int64_t>(number_of_past_steps),
                  initial_time),
...
Errors run_experiment(const StepperBuilder& make_stepper,
  ...
  const auto slab = forward_in_time
                        ? Slab::with_duration_from_start(1., step_magnitude)
                        : Slab::with_duration_to_end(1., step_magnitude);
  const TimeDelta time_step = (forward_in_time ? 1 : -1) * slab.duration();
  const Time initial_time = forward_in_time ? slab.start() : slab.end();
  ...
  errors.volume =
      numerical_volume -
      volume_solution(initial_volume_value, initial_time_value, final_time);
  ...
  for (const auto& direction : directions) {
    ...
    errors.boundary[direction] =
        numerical_boundary - boundary_solution(direction,
...
void test_empty_no_op(const StepperBuilder& make_stepper, const size_t order,
  ...
  const double initial_volume_value = 0.5;
  const Errors coarse = run_experiment(make_stepper, no_directions, true, 0.1,
                                       1, initial_volume_value, 0., false);
...
void test_dense_output() {
  ...
  const auto dense_error = [&](const size_t steps_per_unit) {
    ...
    const auto slab = Slab::with_duration_from_start(
        1.,
        static_cast<double>(total_steps) / static_cast<double>(steps_per_unit));
...
SPECTRE_TEST_CASE("Unit.Time.Actions.SelfStart.BoundaryVariables",
                  "[Unit][Time][Actions]") {
  register_classes_with_charm<TimeSteppers::AdamsBashforth,
  ...
  for (const bool forward_in_time : {true, false}) {
    for (size_t order = 1; order < 5; ++order) {
      ...
      boundary_variables_stepping::test_convergence(
          "AdamsBashforth", make_ab, order, 0.25, forward_in_time);
    }
    for (const size_t order : {size_t{2}, size_t{4}}) {
      ...
      boundary_variables_stepping::test_convergence(
          "AdamsMoultonPc", make_ampc, order, 0.25, forward_in_time);
    }
    {
      const auto make_rk3 = []() {
        return std::make_unique<TimeSteppers::Rk3HesthavenSsp>();
      };
      boundary_variables_stepping::test_convergence("Rk3HesthavenSsp", make_rk3,
...

  {
    ...
    boundary_variables_stepping::test_empty_no_op(make_ab, 2, 0.25);
  }

  boundary_variables_stepping::test_dense_output();
}
