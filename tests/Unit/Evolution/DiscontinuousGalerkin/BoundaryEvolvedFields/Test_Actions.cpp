...
// Writes the per-face dt-stash from the current per-face values, standing in
// for the boundary condition's supplied boundary field time derivative
// in production.
...
struct Component {
  ...
  using step_actions = tmpl::list<
      ...
      evolution::dg::BoundaryEvolvedFields::RecordBoundaryEvolvedFields<
          Dim, field_tags>,
      Actions::MutateApply<UpdateU<system, false>>,
...
void test_multistep_self_start(const size_t order) {
...

  {
    ...
    CHECK_ITERABLE_APPROX(get(get<BoundaryValue<Psi>>(snapshot.at(direction))),
                          node_values);
...

  {
    ...
    CHECK_ITERABLE_APPROX(value_at(), node_values);
  }
...
void test_time_stepper_convergence(const std::string& stepper_label,
  ...
  constexpr size_t Dim = 1;
  constexpr Rate TheRate = Rate::Linear;
  // A signed `step` selects the time direction, mirroring the stock
  // Test_SelfStartActions convergence test.
  const auto error = [&make_stepper,
                      initial_integration_order](const double step) {
    ...
    // substep's update yields the step's value.
    ...
    // Face and volume histories must stay in lockstep for every stepper
    // family: same integration order (the order-sync) and same record count
    // (Record inserts and Clean prunes alongside the volume machinery; for
    // Runge-Kutta this checks substep records were actually discarded).
    ...
    CHECK(face_history.size() == volume_history.size());
...
void test_per_face_corner() {
  ...
  // The load-bearing claim: distinct values at the shared corner node.
...
void test_dense_output() {
  ...
  const Slab slab(0., 1.);
  const auto add = [&history, &p, &dp, number_of_nodes](const Time& time) {
