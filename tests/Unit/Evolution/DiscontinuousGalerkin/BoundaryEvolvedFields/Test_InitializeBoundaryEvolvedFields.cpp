...
// ============================================================================
// InitializeBoundaryEvolvedFields at runtime: on a real Element/Mesh with the
// applied boundary conditions resolved by typeid from
// `ExternalBoundaryConditions`, the mutator allocates entries ONLY for the
// opting external faces (two here) — none for the non-opting external face or
// the interior direction, projects every union field from the volume to each
// face, sizes the dt-stash to the face mesh, and seeds an empty history at
// the volume history's integration order.
// ============================================================================

// A boundary condition that opts in with TWO fields, so the projection of
// every union slot is checked. Derives from the domain base class so the
// mutator's typeid resolution against `ExternalBoundaryConditions` is real.
template <size_t Dim>
...
void test_initialize_boundary_evolved_fields(
  ...
  // Mixed extents so a transposed face ordering would be caught.
  ...
  for (const auto& direction : opting_directions) {
    ...
    tmpl::for_each<tmpl::list<Psi, Pi>>([&face_values, &mesh, &direction,
      ...
      dg::project_tensor_to_boundary(make_not_null(&expected),
                                     ...
                                     direction);
      CHECK_ITERABLE_APPROX(get(get<BoundaryValue<source_tag>>(face_values)),
                            get(expected));
    });
