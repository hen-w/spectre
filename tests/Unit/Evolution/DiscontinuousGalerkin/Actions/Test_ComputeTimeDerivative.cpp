...
template <size_t Dim>
void test_auxiliary(const bool use_moving_mesh) {
  constexpr size_t number_of_grid_points = 5;
  MAKE_GENERATOR(gen);
  UniformCustomDistribution<double> dist{0.0, 1.0};
  ...
  // Call the auxiliary-pass package-data helper, which dispatches to the
  // boundary correction's dg_auxiliary_package_data. This mirrors the physical
  // dg_package_data DataBox call exactly, modulo the helper name.
  db::DataBox<tmpl::list<>> box{};
  const double max_speed =
      ::evolution::dg::Actions::detail::dg_auxiliary_package_data<System<Dim>>(
          make_not_null(&packaged_data), boundary_correction, projected_fields,
          unit_normal_covector, mesh_velocity, box, tmpl::list<>{},
          tmpl::list<Var1,
                     ::Tags::Flux<Var1, tmpl::size_t<Dim>, Frame::Inertial>>{});
  ...
  Variables<tmpl::list<::Tags::NormalDotFlux<Var1>, Var1>>
      expected_packaged_data{number_of_grid_points};
  // Deviation: compare against the boundary correction's
  // dg_auxiliary_package_data.
  const double expected_max_speed =
      boundary_correction.dg_auxiliary_package_data(
          make_not_null(
              &get<::Tags::NormalDotFlux<Var1>>(expected_packaged_data)),
          make_not_null(&get<Var1>(expected_packaged_data)),
          get<Var1>(projected_fields),
...
SPECTRE_TEST_CASE("Unit.Evolution.DG.ComputeTimeDerivative",
  ...
  for (const bool use_moving_mesh : {true, false}) {
    ...
    WithInverseSpatialMetricTag::test_auxiliary<1>(use_moving_mesh);
  }
}
}  // namespace
}  // namespace TestHelpers::evolution::dg::Actions
