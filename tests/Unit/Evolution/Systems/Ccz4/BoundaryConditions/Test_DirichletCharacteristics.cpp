  ...
  struct factory_creation
    ...
    using factory_classes = tmpl::map<
        tmpl::pair<
            ...
            tmpl::list<Ccz4::BoundaryConditions::DirichletCharacteristics>>,
...
void test_creation_and_serialization() {
  ...
  const auto serialized = serialize_and_deserialize(
      *dynamic_cast<Ccz4::BoundaryConditions::DirichletCharacteristics*>(
          boundary_condition.get()));
  CHECK(serialized.get_clone() != nullptr);
}

void test_bc_type() {
  CHECK(Ccz4::BoundaryConditions::DirichletCharacteristics::bc_type ==
...
void test_kerrschild() {
  ...
  // The evolved-field dt corrections are explicitly zeroed by
