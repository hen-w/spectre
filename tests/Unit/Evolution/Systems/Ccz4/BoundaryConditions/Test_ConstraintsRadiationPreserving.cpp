...
template <typename U>
void test_fd(const U& boundary_condition, const size_t max_degree, const bool evolve_lapse_and_shift) {
  ...
  boundary_condition.fd_ghost(
      ...
      int_trace_extrinsic_curvature, int_theta, int_gamma_hat, int_lapse,
      int_shift, int_auxiliary_shift_b, subcell_mesh, reconstructor, evolve_lapse_and_shift);
...
#ifdef SPECTRE_DEBUG
  {
    INFO("Test ConstraintsRadiationPreserving BC with evolve_lapse_and_shift=false");
    const auto product_boundary_condition =
        TestHelpers::test_creation<
            std::unique_ptr<Ccz4::BoundaryConditions::BoundaryCondition>,
            Metavariables>(
