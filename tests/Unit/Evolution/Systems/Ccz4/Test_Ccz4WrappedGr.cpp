...
void test_ccz4_solution(
  ...
  // through verbatim. All of these relations are exact in floating point,
  // so for solutions with positive lapse this loop checks that the
  // wrapper is an exact no-op on the solution's tags.
  ...
  tmpl::for_each<typename SolutionType::template tags<DataVector>>(
      [&vars, &wrapped_vars, &sgn_of_lapse](auto tag_v) {
        ...
        if constexpr (std::is_same_v<tag, LapseTag>) {
          ...
          // Only trivially exercised: no wrapped solution has both a
          // negative lapse and a nonzero time derivative of the lapse
          auto expected = get<tag>(vars);
...
void test_high_spin_kerr_puncture(
  ...
  // solution lapse inside the throat and a positive one outside, and that
  // the lapse derivative is nonzero at the inner points so its sign
  // flip is exercised nontrivially
  using DerivLapse = ::Tags::deriv<gr::Tags::Lapse<DataVector>, tmpl::size_t<3>,
                                   Frame::Inertial>;
  const auto premise_vars = solution.variables(
      x, 0.0, tmpl::list<gr::Tags::Lapse<DataVector>, DerivLapse>{});
  const auto& solution_lapse = get<gr::Tags::Lapse<DataVector>>(premise_vars);
  const auto& solution_deriv_lapse = get<DerivLapse>(premise_vars);
  for (size_t i = 0; i < 3; ++i) {
    CHECK(get(solution_lapse)[i] < 0.0);
    CHECK((solution_deriv_lapse.get(0)[i] != 0.0 or
           solution_deriv_lapse.get(1)[i] != 0.0 or
           solution_deriv_lapse.get(2)[i] != 0.0));
  }
