...
void test_package_data(const gsl::not_null<std::mt19937*> gen) {
  ...
  correction.dg_package_data(
      ...
      pi, boundary_psi, phi, normal_covector, std::nullopt, std::nullopt);
...
void test_boundary_terms(const gsl::not_null<std::mt19937*> gen) {
  ...
  correction.dg_boundary_terms(
      ...
      make_not_null(&boundary_psi_corr), pi_int, ndphi_int, pi_ext, ndphi_ext,
...
void test_auxiliary_package_data(const gsl::not_null<std::mt19937*> gen) {
  ...
  correction.dg_auxiliary_package_data(
      ...
      boundary_psi, normal_covector, std::nullopt, std::nullopt);
...
void test_auxiliary_boundary_terms(const gsl::not_null<std::mt19937*> gen) {
  ...
  correction.dg_auxiliary_boundary_terms(make_not_null(&phi_corr), psi_int,
                                         psn_int, psi_ext, psn_ext,
                                         dg::Formulation::StrongInertial);
...
void test_factory_creation() {
  ...
  normal.get(0) = 1.0;
  serialized.dg_package_data(
      make_not_null(&packaged_pi), make_not_null(&packaged_ndphi), psi, pi,
      boundary_psi, phi, normal, std::nullopt, std::nullopt);
