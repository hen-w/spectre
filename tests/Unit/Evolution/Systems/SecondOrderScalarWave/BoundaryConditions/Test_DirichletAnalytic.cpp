...
struct ConvertPlaneWave {
  ...
  using packed_container = SecondOrderPlaneWave<Dim>;
  using packed_type = double;

  static packed_container create_container() {
    std::array<double, Dim> wave_vector{};
    for (size_t i = 0; i < Dim; ++i) {
      gsl::at(wave_vector, i) = 0.1 + static_cast<double>(i);
    }
    std::array<double, Dim> center{};
    for (size_t i = 0; i < Dim; ++i) {
      gsl::at(center, i) = 1.1 - static_cast<double>(i);
    }
...
void test() {
  ...
  const auto box_analytic_soln = db::create<db::AddSimpleTags<
      Tags::Time, Tags::AnalyticSolution<SecondOrderPlaneWave<Dim>>>>(
  ...
  helpers::test_boundary_condition_with_python<
      ...
      tmpl::list<Tags::AnalyticSolution<SecondOrderPlaneWave<Dim>>>,
      Metavariables<Dim>>(
